...
abstract class DbStorage extends Storage {
	...
	//protected abstract function get_ql($db,$type);
	...
	public function start_writing() {
		...
		if( !$this->backup_storage ) {
			$cn=get_class($this);
			$this->backup_storage=new $cn($this->init_params);
			$this->backup_storage->read_only=1;
		}
		*/
	}                                                                            
	...
	public function rollback() {
		//if( $this->backup_storage ) $this->backup_storage->rollback();
		$this->write_end(0);
		$this->node_cache_list=array();
		$this->node_cache_idx=array();
		$this->triple_cache=array();
		...
		try {
			while($count = count($this->node_cache_list)) {
				...
				if($node->modified) {
					$this->save_node($node);
					/*
					if( array_key_exists($node->id,$this->node_cache_idx) ) {
						$pos=$this->node_cache_idx[$node->id];
						unset($this->node_cache_idx[$node->id]);
						$node2=array_pop($this->node_cache_list);
						if( $node2->id!=$node->id ) {
							$this->node_cache_list[$pos]=$node2;
							$this->node_cache_idx[$node2->id]=$pos;
						}
					}
					*/
				} else {
		...
		if( $prop['c'] && ($prop['n']>8 || (is_string($triple['ob']) && strlen($triple['ob'])>1024)) ) {
			...
			if( array_key_exists($id,$this->triple_cache) ) {
				if( $triple['type']!=='node' ) {
					...
					//throw new \Exception('The triple was already in cache!');
				} else {
					$this->triple_cache[$id]['double']=1;
				}
			}
	...
	public function remove_triple_from_node( &$node, &$triple, $dir=1 ) {
		//$key=($triple['sub']==$node->id?'>':'<').$triple['pred'];
		$key=($dir==1?'>':'<').$triple['pred'];
		if( array_key_exists($key,$node->props) ) {
			$prop=&$node->props[$key];
			$prop['n']--;
	...
	/*
	function find($s,$params=null,$context=null) {
		if( is_null($context) ) $context=$this->tdz_caller_context();
		if( $s ) return $this->get_ql($context)->execute( 'select distinct * where '.$s,$params );
		else return $this->get_ql($context)->execute( 'select distinct *' );
	}
	*/
