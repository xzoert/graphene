...
abstract class NodeBase {
	...
	public function get($n) {
		...
		/*} else {
			$def=null;
			$arr=Syntax::parsePred($n);
			$dir=$arr['dir'];
			$pred=$arr['pred'];
			$mask=\graphene::ACCESS_FULL;
		}*/
		if( $lang ) {
			if( $def && $def->isList ) {
				if( $def->frozen ) throw new \Exception( 'Translations are only supported for single valued properties.' );
...
final class DataNode extends NodeBase {
	...
	public function getTr($n,$lang) {
		$this->validateLang($lang);
		if( $this->type ) $def=$this->type->_findDef($n);
		else $def=null;
		if( !$def ) {
			$arr=Syntax::parsePred($n,$this->ns());
			if( $this->type ) {
				$def=$this->type->_findDefAbs(($arr['dir']==1?'':'@').$arr['pred']);
			}
			$pred=$arr['pred'];
			$dir=$arr['dir'];
		} else {
		...
		if( $def && $def->isList ) throw new \Exception( 'Translations are only supported for single valued properties.' );
		$prop=new Prop($this,$pred.':'.$lang,$dir,\graphene::ACCESS_FULL,$def);
		$r=$prop[0];
		if( is_null($r) ) {
			if( strlen($lang)==5 ) {
				$dprop=new Prop($this,$pred.':'.substr($lang,0,2),$dir,\graphene::ACCESS_FULL,$def);
				$r=$dprop[0];
				if( !is_null($r) ) return $r;
			}
			$dprop=new Prop($this,$pred,$dir,\graphene::ACCESS_FULL,$def);
			return $dprop[0];
		}
		return $r;
	}
	...
	public function setTr($n,$lang,$v) {
		...
		if( Syntax::isEmpty($v) ) {
			...
			throw new \Exception('Can not set multiple translated values.');
		} else {
			$prop[0]=$v;
		} 
		$dprop=new Prop($this,$pred,$dir,\graphene::ACCESS_FULL,$def);
	...
	public function get($n) {
		...
		if( !$def ) {
			...
			if( !$def ) {
				return new Prop($this,$arr['pred'],$arr['dir'],\graphene::ACCESS_FULL);
			}
			$prop=new Prop($this,$arr['pred'],$arr['dir'],\graphene::ACCESS_FULL,$def);
			if( $def->isList ) return $prop;
			else return $prop[0];
		}
		$prop=new Prop($this,$def->predName,$def->dir,\graphene::ACCESS_FULL,$def);
		if( $def->isList ) return $prop;
		else return $prop[0];
	}
	...
	public function set($n,$v) {
		...
		if( Syntax::isEmpty($v) ) {
			...
			if( $def && !$def->isList && count($v)>1 ) throw new \Exception('Property '.$n.' is single valued.');
			$prop->reset($v);
		} else {
			$prop[0]=$v;
		} 
